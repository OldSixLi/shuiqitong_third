    ...
    <header class="topbar">
      <div>
        ...
        <div class="exit-block" >
          <div v-show="userIcon" style="border-radius:50%;border:2px solid #fff;float: left;overflow: hidden;height: 26px;width:26px;">
            <img  class="exit-img" :src="userIcon"  alt="退出" title="用户头像"></div>
            <span class="username" style="vertical-align:top;margin-right:10px;margin-left: 10px;">{{userName||""}}</span>
        ...
        <template v-for="x in menuList">
          <div class="li" :class="{'slide-small':showFullWidth}">
            ...
            <a class="title-menu" :class="{active:currentParentId==x.parentId}" @click="parentClick(x.parentId,$event)">
              ...
              <span class="text" v-show="showFullWidth" style="font-family:'MicroSoft YaHei';font-size: 15px;"><b>{{x.name}}</b></span>
            ...
            <ul class="child-menu" :class="{active:currentParentId==x.parentId,normal:currentParentId!=x.parentId}">
              <li v-for="y in x.childList" @click="setActiveChild(y.pointName)" :class="{active:currentChildPoint==y.pointName}"
                v-if="y.name">
                <router-link :to="y.action||'/'"><span style="font-size: 14px;">{{y.name}}</span></router-link>
              </li>
            </ul>
          </div>
        </template>
...
<script>
  ...
  import { getWindowHeight } from '@/util/dom.js';
