      ...
      <el-col :span="22" :offset="1">
        ...
        <el-row>
          ...
          <el-table ref="tableSet" border :data="tableData" style="min-width: 100%;" :maxHeight="availHeight()"
            ...
            <template v-if="colData.includes('姓名')">
            ...
            <template v-if="colData.indexOf('地址')>-1">
              <el-table-column label="是否回执" align="center" width="100">
                <template slot-scope="scope">
                  ...
                  <el-button type="text" @click="toReadList(scope.row)" title="查看回执" v-if="scope.row.TYPE=='0'"
                    :disabled="scope.row.STATE=='0'">查看回执</el-button>
                  <!-- <a style="cursor:pointer;" class="title-a" @click="toReadList(scope.row)" title="查看回执" > <span >查看回执</span> </a> -->
            ...
            <el-table-column fixed="right" label="操作" width="150" align="left">
              <template slot-scope="scope">
                ...
                <icon-btn :circle="true" @clicks="sendMsg(scope.row)" :src="require('@/assets/img/sendMsg.png')" word="发送消息"
                  :loading="currentLoadId==scope.row.ID" v-show="scope.row.STATE=='0'"></icon-btn>
                <icon-btn style="margin-left:10px;" slot="reference" :circle="true" @clicks="deleteClick($event,scope.row)"
                  :src="require('@/assets/img/delete.png')" word="删除消息"></icon-btn>
              </template>
            </el-table-column>
          </el-table>
        ...
        <el-dialog title="消息详情" :visible.sync="showInfoModal" :modal-append-to-body='false' @close="modalClose">
          <div style="margin:0 30px;">
            ...
            <el-row :gutter="20" class="detail-row">
              ...
              <el-col :span="14" class="right-panel">
                <el-input type="textarea" :autosize="{ minRows: 2, maxRows: 8}" :rows="5" v-model="detailInfo.CONTENT"
            ...
            <el-row :gutter="20" class="detail-row">
              ...
              <el-col :span="14" class="right-panel">
                <el-input :value="detailInfo.createName" :disabled="true"></el-input>
              </el-col>
            </el-row>
            <el-row :gutter="20" class="detail-row">
            ...
            <el-row :gutter="20" class="detail-row">
              ...
              <el-col :span="14" class="right-panel">
                ...
                <el-button type="text" :disabled="detailInfo.STATE=='0'" v-if="detailInfo.TYPE=='0'" @click="toReadList(detailInfo)">查看回执</el-button>
              </el-col>
            </el-row>
          </div>
        ...
        ..:::::..::........:::........:::..:::::..:::.......:::........:::..:::::..::........::
        -->
        <el-dialog title="新建消息" :visible.sync="addModal" :modal-append-to-body='false' :close-on-click-modal="false">
          <!--  -->
          <div>
            <el-row :gutter="20" class="detail-row">
              <el-col :span="6" class="text-right title">消息标题</el-col>
              <el-col :span="14" class="right-panel">
                <el-input v-model.trim="detailInfo.TITLE" placeholder="请输入消息标题"></el-input>
              </el-col>
            </el-row>
            <el-row :gutter="20" class="detail-row">
              <el-col :span="6" class="text-right title">消息内容</el-col>
              <el-col :span="14" class="right-panel">
                <el-input :rows="5" type="textarea" :autosize="{ minRows: 2, maxRows: 8}" v-model.trim="detailInfo.CONTENT"
                  :maxlength="1000" placeholder="请输入消息内容"></el-input>
                <span style="margin:0;float:right;color: #909399;">剩余
                  {{1000-((detailInfo.CONTENT&&detailInfo.CONTENT.length)||0)}} 字</span>
              </el-col>
            </el-row>
            <!--
            :'######:::'#######::'##::::'##:'########:::::'###::::'##::: ##:'##:::'##:
            ...
            :......::::.......:::..:::::..::..:::::::::..:::::..::..::::..:::::..:::::
            -->
            <el-row :gutter="20" class="detail-row" style="margin-top:-10px;">
              <el-col :span="6" class="text-right title">
                <el-popover placement="top" title="企业筛选" width="200" trigger="click" content="开启企业筛选后,只有选择列表中企业的员工可以接收到此消息">
              ...
              <el-col :span="14" class="right-panel" style="line-height:38px;">
                <el-switch v-model="isSelectCom" active-text="筛选企业" inactive-text="全部企业">
                </el-switch>
              </el-col>
            </el-row>
            <transition name="el-zoom-in-top">
              <el-row :gutter="20" class="detail-row" v-show="isSelectCom">
                <el-col :span="6" class="text-right title">选择公司</el-col>
                <el-col :span="14" :offset="0">
                  <!-- {{selectedComIds}} -->
                  <com-slt @result="getComSltResult" :com-ids="selectedComIds" :btn-text="comBtnText"></com-slt>
                </el-col>
              </el-row>
            </transition>
            <!--
...
<script>
  ...
  import comSlt from '@/components/comSelect'
  //组件逻辑
  export default {
    name: 'QusReplyList',
    components: {
    ...
    methods: {
      ...
      checkMsg() {
        if (!this.detailInfo.TITLE) {
          this.$message.error("请输入消息标题！");
          return false;
        } else if (this.detailInfo.TITLE.length > 500) {
          this.$message.error("您输入的消息标题超出500字，请修改！");
          return false;
        }
        if (!this.detailInfo.CONTENT) {
          this.$message.error("请输入消息内容！");
          return false;
        } else if (this.detailInfo.TITLE.length > 1000) {
          this.$message.error("您的消息内容过长，请修改！");
          return false;
        }

        if (this.isSelectCom && this.selectedComIds.length == 0) {
          this.$message.error("您开启了企业筛选，请选择企业！");
          return false;
        }
        if (this.isAccuratePush && this.checkedRoles.length == 0) {
          this.$message.error("您开启了精准推送，请选择角色！");
          return false;
        }
      ...
      saveAndSend() {
        ...
        this.$post(`/tax/message/saveAndSend`, {
          ...
          enterpriseIds: this.selectedComIds.join(',')
        }).then(x => {
          if (x.success) {
            this.$message({
              message: '发送消息成功!',
    ...
    watch: {
      ...
      "selectedComIds": {
        ...
        immediate: true,
        handler: function () {
          if (this.selectedComIds.length > 0) {
            this.comBtnText = `已选择${this.selectedComIds.length}家企业`;
          } else {
            this.comBtnText = `请选择企业`;
          }
        }
      }
    },
