  ...
  <div>
    ...
    <el-row style="margin-top: 15px;">
      <el-col :span="22" :offset="1"> 
        <p class="table-title text-right" style="padding:3px 10px;">
        <span style="float:left;line-height: 35px;">问卷调查结果列表</span>  
          <el-button type="primary" size="small" @click="backStatistics" >返回调查结果列表</el-button>
        </p>
                    ...
                    <div class="form-group chooseContain" style="padding-left: 20px;padding-right: 20px;">
                      <ul v-for="(y,yIndex) in x.options" v-show="x.question.typeArr&&(x.question.typeArr.indexOf('1')>-1||x.question.typeArr.indexOf('2')>-1)">
                        <li style="list-style: none; ">
                          <el-radio disabled :label="y.id.toString()" :key="y.id" v-model="x.optionAnswer" readonly v-if="x.question.typeArr&&(x.question.typeArr.indexOf('1')>-1)">{{''}}</el-radio>
                          <el-checkbox disabled readonly v-if="x.question.typeArr&&(x.question.typeArr.indexOf('2')>-1)"
...
<script>
  ...
  export default {
    ...
    methods: {
      backStatistics(){
        this.qnaId&&this.$to(`/quan/statistics`);
      },
      indexMethod(index) {
        ...
        this.$post(`/tax/question/anwerDetail`, {
          ...
          .then(data => {
            if (data.success) {
              ...
              this.$message.error(data.message||"未获取到数据"); 
              this.detailModal = false;
            }
          }).catch((x) => {
            console.log(x);
