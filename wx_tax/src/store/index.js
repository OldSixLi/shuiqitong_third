...
const store = new Vuex.Store({
  ...
  state: {
    isLogin: true,
    token: token|| mystorage.getItem('shuiqitong_token') ||"" ,
    comInfo: {}, // 微信相关信息
    userInfo: {}, // 微信相关信息
     //0:审核中 1:审核通过  2:审核失败 3:重新提交审核  4:未提交审核
    checkState:mystorage.getItem('checkState')||4,
    failReason:"",
    departmentId: 0,
    currentSelectRole: "",
    currentSelectRoleName: "",
    checkValue: "", //部门选择人员ID
    companyId: mystorage.getItem('currenComId') || wxComId || "", //微信公司ID 
    comUserId: mystorage.getItem('comUserId') || wxUserId || "", //员工在此公司微信的ID 
  ...
  mutations: {
    ...
    changeToken(state, token) {
      if (token != state.token) {
        debugger
        mystorage.setItem('shuiqitong_token',token);
    ...
    changeCheckState(state, currentState, reason = "") {
      ...
      mystorage.setItem('checkState',currentState);
    ...
    setComId(state, value) {
      mystorage.setItem('currenComId', value);
    ...
    setComUserId(state, value) {
      mystorage.setItem('comUserId', value);
